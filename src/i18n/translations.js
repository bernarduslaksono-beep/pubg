export const LANGUAGES = [
  { code: 'tet', label: 'Tet' },
  { code: 'en', label: 'Eng' },
  { code: 'id', label: 'Ind' },
]

export const DEFAULT_LANGUAGE = 'tet'

export const TRANSLATIONS = {
  tet: {
    nav_home: "Uma",
    footer_nav_label: "Navigasaun",
    nav_order: "Sosa",
    portal_title: "Hili Jogu",
    portal_desc: "Top up ba pakote UC, Diamonds no seluk tan, seguru no lalais.",
    portal_topup_btn: "Halo Top Up",
    portal_hero_title: "TOP UP GAME & VOUCHER",
    how_it_works_title: "Oinsá Ne'e Funsiona",
    trust_section_title: "Tanba Saida Halo Top Up Iha Ami",
    coming_soon: "Sei Mai",
    visitor_label: "Total Vizitante",
    trust_completed_label: "Pedidu Kompletu Ona",
    trust_satisfaction_label: "Kliente Satisfeitu",
    footer_payment_label: "Metode Pagamentu",
    footer_contact_label: "Kontaktu Ami",
    footer_settings_label: "Konfigurasaun",
    footer_whatsapp_prefix: "WhatsApp Support",
    nav_track: "Haree Status",
    footer_text: "Loja-Game · Dili, Timor-Leste",

    hero_title: "Top up UC PUBG Global",
    shop_title: "Top up UC PUBG Global",
    hero_title_for: (g) => `Top up ${g}`,
    shop_title_for: (g) => `Top up ${g}`,
    step1: "Hili pakote",
    step2: "Halo transferénsia",
    step3: "Upload prova transferénsia",
    step4: "Kria pedidu",
    hero_desc: "Ami sei verifika no haruka pakote ba game ID ita boot nian (Prosesu ±15 menit).",
    hero_desc_roblox: "Ami sei verifika no haruka Redeem Code iha itaboot nia pedidu ne, favor cek iha menu Hare status. (Prosesu ±15 menit).",
    whatsapp_confirm: "WhatsApp Konfirmasaun",

    step_indicator_pick: "Hili Pakote",
    step_indicator_pay: "Pagamentu",

    shop_avail: "Disponivel ba Timor-Leste",
    promo_1: "Free antar Dili laran",
    promo_2: "Prosesu haruka ±15 menit",
    promo_3: "Pagamentu seguru — Bank BNCTL / MOSAN",
    trust_delivery: "Prosesu ±15 Menit",
    trust_secure: "Pagamentu Seguru",
    trust_support: "Suporta WhatsApp",
    search_pkg_placeholder: "Buka pakote (ezemplu: 1000)",
    order_history_label: "Pedidu ikus ne'e",
    order_history_empty: "Seidauk iha pedidu iha device ne'e.",
    select_product_label: "Hili Produtu",
    pkg_count_suffix: "pakote",
    out_of_stock_label: "Stok Hotu",

    tier_kiik: "Pakote Ki'ik",
    tier_medium: "Pakote Medium",
    tier_boot: "Pakote Bo'ot",
    tier_boot_note: "(Prosesu ±30 menit)",

    checkout_process_time: "Prosesu haruka: ±15 menit",
    about_title: "Kona ba Ami",
    about_story_title: "Istória Ami Nian",
    about_need_help_title: "Presiza Ajuda?",
    about_need_help_desc: "Se ita boot iha pergunta kona ba pedidu, pagamentu, ka status pedidu, favor kontaktu ami liu husi WhatsApp.",
    about_facebook_label: "Haree lista-lista presu ami nian husi tinan-tinan liu ba →",
    footer_about_label: "Kona ba Ami",
    about_paragraphs: [
      "Dezde tinan 2018, ami hah\u00fa fa'an UC PUBG liu husi entrega diretu ba uma kliente sira nian iha Dili. Iha tempu ne'eb\u00e1, ami seidauk iha website \u2014 ami uza de'it banner no lista presu ne'ebe ami troka beibeik tuir Royale Pass Season foun-foun, hafoin haruka liu husi WhatsApp no lori diretu ba kliente nia liman rasik.",
      "Razaun forte husi primeiru ami hah\u00fa fa'an UC mak tanba ami hakarak ajuda gamer sira Timor oan atu bele hetan fasilidade lalais atu hadi'a Royale Pass, hanesan mos nasaun sira seluk.",
      "Iha tinan 2020, durante pandemia COVID-19, ami tenki para servisu ne'e durante tinan ida \u2014 hanesan barak seluk iha Timor-Leste. Maibe hafoin situasaun hadi'a, ami hakarak fila fali no kontinua servisu ne'e, tanba ami hatene ita boot sira mos sei hein hela ami.",
      "Durante tinan hira ne'e, ami hadia lista presu no deze\u00f1u ami nian dala barak \u2014 komesa husi Season 4 to'o Season 11 \u2014 atu sempre f\u00f3 presu justu no informasaun klaru ba kliente. Ida ne'e la'os neg\u00f3siu foun ka konfiansa ne'ebe ami hetan lais de'it; ida ne'e resultadu husi tinan-tinan servisu konsistente ba komunidade Dili.",
      "Agora, ami lori servisu ne'e ba online \u2014 la'os atu muda buat ida, maibe atu halo prosesu sai fasil liu tan ba ita boot no presu sai barato liu. Ema ne'ebe verifika pedidu ita boot nian hanesan ema ne'ebe lori UC ba uma kliente sira durante tinan hira ne'e.",
      "Iha website ida ne'e, ami mos ona hatama pakote ba Mobile Legends, Free Fire, no Robux Roblox \u2014 no ami sei kontinua hatama game foun sira seluk iha futuru, atu bele responde ba nesesidade komunidade gamer Timor oan nian ne'ebe sempre aumenta."
    ],
    redeem_howto_title: "Oinsa redeem code Roblox?",
    redeem_step1: "Loke aplikasu Roblox ka website roblox.com, hafoin tama ho ita boot nia konta.",
    redeem_step2: "Iha parte leten, klik ikon kaixa/prezente 📦 ka menu (☰).",
    redeem_step3: "Hili opsaun \"Redeem Roblox Codes\" — ka bele diretamente ba website: roblox.com/redeem",
    redeem_step4: "Hatama kódigu ne'ebe admin fo iha kotak \"Code\".",
    redeem_step5: "Klik butaun \"Redeem\".",
    redeem_step6: "Robux sei tama automátikamente ba balansu ita boot nia konta.",

    selected_pkg_label: "Pakote hili",
    price_per_unit_label: "Osan / unidade",
    select_pkg_empty: "Hili pakote",

    user_id_label: "User ID PUBG Global",
    user_id_label_generic: "User ID",
    user_id_placeholder_generic: "Ejemplu: 123456789",
    zone_id_label: "Zone ID",
    zone_id_placeholder: "Ejemplu: 2151",
    user_id_placeholder: "Ejemplu: 1234567891234567",
    user_id_placeholder_suffix: "234567891234567",
    user_id_must_start_5: "User ID tenki hahu ho numeru 5.",
    nickname_label: "Nickname",
    nickname_placeholder: "Ejemplu: ucpubgtl2026",
    quantity_label: "Kuantidade",
    qty_decrease_label: "Hamenus kuantidade",
    qty_increase_label: "Aumenta kuantidade",
    fullname_label: "Naran completu",
    fullname_placeholder: "Naran ita boot",
    whatsapp_label: "Numeru WhatsApp",
    whatsapp_placeholder: "7XXXXXXX",
    add_note_btn: "+ Hatama nota ba seller (opsional)",
    note_label: "Nota ba seller",
    note_placeholder: "Informasaun adisional",

    total_uc_label: "Total",
    subtotal_label: "Subtotal",
    buy_btn: "Sosa",
    buy_hint_select_pkg: "Hili pakote uluk atu bele kontinua.",
    buy_hint_fill_info: "Kompleta informasaun jogador uluk atu bele kontinua.",

    payment_method_title: "Metode Pagamentu",
    type_bank_transfer: "Transferénsia",
    type_ewallet: "E-Wallet",

    upload_title: "Upload Prova Transferénsia",
    upload_hint: "Transfere osan ba ami nia konta, depois upload prova transferénsia iha ne'e.",
    upload_locked_hint: "Hili metode pagamentu uluk antes upload prova transferénsia.",
    upload_locked_main: "Hili metode pagamentu uluk",
    upload_main: "Upload prova transferénsia",
    upload_sub: "PNG ka JPG, até 5MB",
    upload_btn: "Hili Imajen",
    upload_file_ok: "Imajen hili ona",
    upload_change_btn: "Troka imajen",

    order_info_title: "Informasaun Pedido",
    edit_link: "Edit",

    payment_details_title: "Detalhus Pagamentu",
    total_order_label: "Total Pedidu",
    total_payment_label: "Total Pagamentu",

    submit_btn: "Haruka Pedidu",
    submitting_label: "Haruka...",
    submit_error: "Falha atu haruka pedidu. Favor tenta fila fali.",
    submit_hint_select_payment: "Hili metode pagamentu uluk atu bele haruka pedidu.",
    submit_hint_upload_proof: "Upload prova transferénsia uluk atu bele haruka pedidu.",

    success_title: "Pedidu Submete!",
    success_desc: (currency, wa) => `Ami sei verifika pagamentu no haruka ${currency} ba game ID ita boot nian. Guarda Order ID ne'e atu haree status, no konfirma liu husi WhatsApp ${wa}.`,
    success_desc_roblox: (wa) => `Ami sei verifika pagamentu no haruka kódigu redeem ba ita boot. Guarda Order ID ne'e, hafoin cek iha menu Hare Status atu simu kódigu redeem, ka konfirma liu husi WhatsApp ${wa}.`,
    copy_btn: "Copy",
    copied_btn: "Tersalin",
    ok_btn: "Diak",

    track_eyebrow: "Status Pedidu",
    track_title_line1: "Haree progress",
    track_title_line2: "UC ita boot nian.",
    track_desc: "Hatama Order ID atu buka pedidu ita boot. Ita boot hetan Order ID hafoin submete pedidu.",
    order_id_label: "Order ID",
    order_id_placeholder_example: (prefix) => `cth. ${prefix}-M2K3X9`,
    optional_label: "(opsional)",
    track_btn: "Haree Status",
    track_loading: "Buka...",
    track_empty_input: "Favor hatama Order ID.",
    track_not_found: "Pedidu la hetan. Favor verifika Order ID ne'ebe ita boot hatama.",
    track_error: "Falha atu buka pedidu. Favor tenta fila fali.",
    track_history_loading: "Buka riwayat pedidu...",
    track_history_empty: "Seidauk iha pedidu iha dispositivu ida-ne'e. Pedidu ne'ebe ita boot halo sei aparese iha ne'e.",
    track_contact_whatsapp_btn: "Kontaktu WhatsApp",
    unread_badge_label: "Foun",
    notification_bell_label: "Notifikasaun",
    notification_bell_label_unread: "Notifikasaun — iha atualizasaun foun",
    notification_bell_dropdown_title: "Iha atualizasaun:",
    update_available_message: "Iha versaun foun ba website ne'e.",
    update_now_btn: "Update Agora",
    close_btn: "Taka",
    install_ios_title: "Hatama Loja-Game ba Ecrã Inísiu",
    install_ios_sub: "Tap ikon Share ⬆️ iha Safari, hafoin hili \"Add to Home Screen\"",
    install_title: "Install Loja-Game",
    install_sub: "Hatama app ne'e ba ecrã inísiu ita boot nian",
    install_btn: "Install",
    theme_light_label: "Modu Naroman",
    theme_dark_label: "Modu Nakukun",
    scroll_to_top_label: "Subi ba leten",
    order_detail_title: "Detallu Pedidu",

    track_explain_menunggu_verifikasi: "Ami sei verifika pagamentu ita boot nian. La presiza halo tan buat ida — hein de'it, no atualiza pájina ida-ne'e atu haree progress.",
    track_explain_terverifikasi: "Pagamentu ona konfirma. Ami prepara atu haruka pedidu ita boot nian.",
    track_explain_terkirim: "Pedidu ona kompletu no haruka ona. Obrigadu tanba sosa iha Loja-Game!",
    track_explain_dibatalkan: "Pedidu ida-ne'e kanseladu. Haree nota husi seller iha kraik se iha, ka kontaktu ami liu husi WhatsApp se presiza ajuda.",

    pkg_row_label: "Pakote",
    price_row_label: "Osan",
    game_id_row_label: "Game ID",
    pubg_name_row_label: "Nickname",
    date_row_label: "Data",
    seller_note_label: "Nota husi seller",
    rating_question: "Satisfeitu ho servisu ida-ne'e?",
    rating_thanks: "Obrigadu ba ita boot nia feedback!",
    rating_comment_prompt: "Bele fó hatene ami se iha buat ne'ebe la diak? (opsional)",
    rating_comment_placeholder: "Hakerek ituan iha ne'e...",
    rating_skip_btn: "La iha",
    rating_send_btn: "Haruka",
    rating_option_bad: "La diak",
    rating_option_neutral: "Neutru",
    rating_option_good: "Kontente",
    security_pending_limit: "Aktualmente iha ita boot nia pedidu 3 seidauk prosesu (Hein Verifikasaun). Favor hein to'o pedidu sira ne'e prosesu ona, hafoin bele halo pedidu foun.",
    security_cancelled_limit: "Ohin loron ita boot nia pedidu 5 ona kanseladu. Ita boot bele halo pedidu foun aban, ka kontaktu admin ba informasaun liu tan.",
    security_blocked: "Ita boot nia dispositivu seidauk bele halo pedidu foun. Favor kontaktu admin liu husi WhatsApp ba informasaun liu tan.",
    store_closed_banner: (openTime) => `Agora dadaun loja taka hela, ami sei loke fila fali iha oras tuku ${openTime} dader. Ita boot bele haree pakote no presu, maibe seidauk bele halo pedidu.`,
    store_closed_error: (openTime) => `Loja taka. Ami sei loke fila fali iha oras ${openTime}. Favor tenta fali bainhira loja loke ona.`,
    proof_duplicate_error: (orderId, wa) => `Bukti transferénsia ne'e ona uza ba pedidu ${orderId} ne'ebe seidauk kanselamentu. Se ita boot iha problema, favor kontaktu admin liu husi WhatsApp ${wa}.`,
    refresh_status_label: "Atualiza status",

    status: {
      menunggu_verifikasi: "Hein Verifikasaun",
      terverifikasi: "Verifikadu",
      terkirim: "Haruka Ona",
      dibatalkan: "Kanseladu",
    },
  },

  en: {
    nav_home: "Home",
    footer_nav_label: "Navigation",
    nav_order: "Order",
    portal_title: "Choose Your Game",
    portal_desc: "Top up UC, Diamonds and more, safe and fast.",
    portal_topup_btn: "Top Up Now",
    portal_hero_title: "TOP UP GAME & VOUCHER",
    how_it_works_title: "How It Works",
    trust_section_title: "Why Top Up With Us",
    coming_soon: "Coming Soon",
    visitor_label: "Total Visitors",
    trust_completed_label: "Completed Orders",
    trust_satisfaction_label: "Satisfied Customers",
    footer_payment_label: "Payment Methods",
    footer_contact_label: "Contact Us",
    footer_settings_label: "Settings",
    footer_whatsapp_prefix: "WhatsApp Support",
    nav_track: "Check Status",
    footer_text: "Loja-Game · Dili, Timor-Leste",

    hero_title: "Top up UC PUBG Global",
    shop_title: "Top up UC PUBG Global",
    hero_title_for: (g) => `Top up ${g}`,
    shop_title_for: (g) => `Top up ${g}`,
    step1: "Choose a package",
    step2: "Make the transfer",
    step3: "Upload proof of transfer",
    step4: "Submit the order",
    hero_desc: "We'll verify payment and send your package to your game ID (Processing time ±15 minutes).",
    hero_desc_roblox: "We will verify payment and send the Redeem Code to your order, please check the Check Status menu. (Processing time ±15 minutes).",
    whatsapp_confirm: "WhatsApp Confirmation",

    step_indicator_pick: "Choose Package",
    step_indicator_pay: "Payment",

    shop_avail: "Available in Timor-Leste",
    promo_1: "Free delivery within Dili",
    promo_2: "Delivery time ±15 minutes",
    promo_3: "Secure payment — Bank BNCTL / MOSAN",
    trust_delivery: "±15 Min Processing",
    trust_secure: "Secure Payment",
    trust_support: "WhatsApp Support",
    search_pkg_placeholder: "Search package (e.g. 1000)",
    order_history_label: "Recent orders",
    order_history_empty: "No orders on this device yet.",
    select_product_label: "Select Product",
    pkg_count_suffix: "packages",
    out_of_stock_label: "Out of Stock",

    tier_kiik: "Small Packages",
    tier_medium: "Medium Packages",
    tier_boot: "Large Packages",
    tier_boot_note: "(Processing ±30 minutes)",

    checkout_process_time: "Delivery time: ±15 minutes",
    about_title: "About Us",
    about_story_title: "Our Story",
    about_need_help_title: "Need Help?",
    about_need_help_desc: "If you have questions about your order, payment, or order status, please contact us via WhatsApp.",
    about_facebook_label: "See our price lists from over the years →",
    footer_about_label: "About Us",
    about_paragraphs: [
      "Since 2018, we started selling PUBG UC through direct delivery to clients' homes in Dili. Back then, we didn't have a website yet \u2014 we just used banners and price lists that we updated regularly following each new Royale Pass Season, then sent them via WhatsApp and delivered them directly into the client's hands.",
      "The strong reason we first started selling UC was because we wanted to help young Timorese gamers get quick access to upgrade their Royale Pass, just like players in other countries.",
      "In 2020, during the COVID-19 pandemic, we had to pause this service for a year \u2014 like many others in Timor-Leste. But once things improved, we wanted to come back and continue, because we knew you were still waiting for us.",
      "Over the years, we've refined our price lists and designs many times \u2014 from Season 4 all the way to Season 11 \u2014 always aiming to give fair prices and clear information to customers. This isn't a new business or trust we gained overnight; it's the result of years of consistent service to the Dili community.",
      "Now, we've brought this service online \u2014 not to change anything, but to make the process easier for you and the prices even more affordable. The person verifying your order is the same person who delivered UC to customers' homes all these years.",
      "On this website, we've also added packages for Mobile Legends, Free Fire, and Robux Roblox \u2014 and we'll keep adding more games in the future, to keep up with the growing needs of the Timorese gaming community."
    ],
    redeem_howto_title: "How to redeem a Roblox code?",
    redeem_step1: "Open the Roblox app or roblox.com, then log in to your account.",
    redeem_step2: "At the top, click the gift/box icon 📦 or the menu (☰).",
    redeem_step3: "Select \"Redeem Roblox Codes\" — or go directly to: roblox.com/redeem",
    redeem_step4: "Enter the code admin sent you in the \"Code\" box.",
    redeem_step5: "Click the \"Redeem\" button.",
    redeem_step6: "Robux will be added to your account balance automatically.",

    selected_pkg_label: "Selected package",
    price_per_unit_label: "Price / unit",
    select_pkg_empty: "Choose a package",

    user_id_label: "User ID PUBG Global",
    user_id_label_generic: "User ID",
    user_id_placeholder_generic: "Example: 123456789",
    zone_id_label: "Zone ID",
    zone_id_placeholder: "Example: 2151",
    user_id_placeholder: "Example: 1234567891234567",
    user_id_placeholder_suffix: "234567891234567",
    user_id_must_start_5: "User ID must start with the number 5.",
    nickname_label: "Nickname",
    nickname_placeholder: "Example: ucpubgtl2026",
    quantity_label: "Quantity",
    qty_decrease_label: "Decrease quantity",
    qty_increase_label: "Increase quantity",
    fullname_label: "Full name",
    fullname_placeholder: "Your name",
    whatsapp_label: "WhatsApp number",
    whatsapp_placeholder: "7XXXXXXX",
    add_note_btn: "+ Add a note for the seller (optional)",
    note_label: "Note for seller",
    note_placeholder: "Additional information",

    total_uc_label: "Total",
    subtotal_label: "Subtotal",
    buy_btn: "Buy",
    buy_hint_select_pkg: "Choose a package first to continue.",
    buy_hint_fill_info: "Fill in your player info first to continue.",

    payment_method_title: "Payment Method",
    type_bank_transfer: "Bank Transfer",
    type_ewallet: "E-Wallet",

    upload_title: "Upload Proof of Transfer",
    upload_hint: "Transfer the payment to our account, then upload proof of transfer here.",
    upload_locked_hint: "Choose a payment method first before uploading proof of transfer.",
    upload_locked_main: "Choose a payment method first",
    upload_main: "Upload proof of transfer",
    upload_sub: "PNG or JPG, up to 5MB",
    upload_btn: "Choose Image",
    upload_file_ok: "Image selected",
    upload_change_btn: "Change image",

    order_info_title: "Order Information",
    edit_link: "Edit",

    payment_details_title: "Payment Details",
    total_order_label: "Order Total",
    total_payment_label: "Total Payment",

    submit_btn: "Submit Order",
    submitting_label: "Submitting...",
    submit_error: "Failed to submit order. Please try again.",
    submit_hint_select_payment: "Choose a payment method first to submit your order.",
    submit_hint_upload_proof: "Upload proof of transfer first to submit your order.",

    success_title: "Order Submitted!",
    success_desc: (currency, wa) => `We'll verify your payment and send ${currency} to your game ID. Save this Order ID to check status, and confirm via WhatsApp ${wa}.`,
    success_desc_roblox: (wa) => `We'll verify your payment and send you a redeem code. Save this Order ID, then check the Check Status menu to receive your redeem code, or confirm via WhatsApp ${wa}.`,
    copy_btn: "Copy",
    copied_btn: "Copied",
    ok_btn: "OK",

    track_eyebrow: "Order Status",
    track_title_line1: "Check the progress",
    track_title_line2: "of your UC.",
    track_desc: "Enter your Order ID to find your order. You received the Order ID after submitting your order.",
    order_id_label: "Order ID",
    order_id_placeholder_example: (prefix) => `e.g. ${prefix}-M2K3X9`,
    optional_label: "(optional)",
    track_btn: "Check Status",
    track_loading: "Loading...",
    track_empty_input: "Please enter an Order ID.",
    track_not_found: "Order not found. Please check the Order ID you entered.",
    track_error: "Failed to load order. Please try again.",
    track_history_loading: "Loading order history...",
    track_history_empty: "No orders yet on this device. Orders you place will appear here.",
    track_contact_whatsapp_btn: "Contact WhatsApp",
    unread_badge_label: "New",
    notification_bell_label: "Notifications",
    notification_bell_label_unread: "Notifications — new updates",
    notification_bell_dropdown_title: "Updates:",
    update_available_message: "A new version of this website is available.",
    update_now_btn: "Update Now",
    close_btn: "Close",
    install_ios_title: "Add Loja-Game to Home Screen",
    install_ios_sub: "Tap the Share icon ⬆️ in Safari, then choose \"Add to Home Screen\"",
    install_title: "Install Loja-Game",
    install_sub: "Add this app to your home screen",
    install_btn: "Install",
    theme_light_label: "Light mode",
    theme_dark_label: "Dark mode",
    scroll_to_top_label: "Scroll to top",
    order_detail_title: "Order Details",

    track_explain_menunggu_verifikasi: "We're verifying your payment. No action needed — just wait, and refresh this page to check progress.",
    track_explain_terverifikasi: "Your payment has been confirmed. We're preparing to deliver your order.",
    track_explain_terkirim: "Your order is complete and has been delivered. Thanks for shopping at Loja-Game!",
    track_explain_dibatalkan: "This order has been cancelled. Check the note from the seller below if there is one, or contact us via WhatsApp if you need help.",

    pkg_row_label: "Package",
    price_row_label: "Price",
    game_id_row_label: "Game ID",
    pubg_name_row_label: "Nickname",
    date_row_label: "Date",
    seller_note_label: "Note from seller",
    rating_question: "Satisfied with this service?",
    rating_thanks: "Thanks for your feedback!",
    rating_comment_prompt: "Can you tell us what went wrong? (optional)",
    rating_comment_placeholder: "Write a little here...",
    rating_skip_btn: "Skip",
    rating_send_btn: "Send",
    rating_option_bad: "Not good",
    rating_option_neutral: "Neutral",
    rating_option_good: "Satisfied",
    security_pending_limit: "You currently have 3 orders still awaiting verification. Please wait until they are processed before placing a new order.",
    security_cancelled_limit: "You have had 5 cancelled orders today. You can order again tomorrow, or contact admin for more information.",
    security_blocked: "Your device is currently unable to place new orders. Please contact admin via WhatsApp for more information.",
    store_closed_banner: (openTime) => `The store is currently closed. We will reopen at ${openTime}. You can browse packages and prices, but ordering is not available yet.`,
    store_closed_error: (openTime) => `The store is closed. We will reopen at ${openTime}. Please try again once the store is open.`,
    proof_duplicate_error: (orderId, wa) => `This proof of transfer has already been used for order ${orderId}, which has not been cancelled. If there is an issue, please contact admin via WhatsApp ${wa}.`,
    refresh_status_label: "Refresh status",

    status: {
      menunggu_verifikasi: "Awaiting Verification",
      terverifikasi: "Verified",
      terkirim: "Delivered",
      dibatalkan: "Cancelled",
    },
  },

  id: {
    nav_home: "Beranda",
    footer_nav_label: "Navigasi",
    nav_order: "Pesan",
    portal_title: "Pilih Game",
    portal_desc: "Top up UC, Diamond, dan lainnya, aman dan cepat.",
    portal_topup_btn: "Top Up Sekarang",
    portal_hero_title: "TOP UP GAME & VOUCHER",
    how_it_works_title: "Cara Kerja",
    trust_section_title: "Kenapa Top Up di Sini",
    coming_soon: "Segera Hadir",
    visitor_label: "Total Pengunjung",
    trust_completed_label: "Pesanan Selesai",
    trust_satisfaction_label: "Pelanggan Puas",
    footer_payment_label: "Metode Pembayaran",
    footer_contact_label: "Hubungi Kami",
    footer_settings_label: "Pengaturan",
    footer_whatsapp_prefix: "WhatsApp Support",
    nav_track: "Cek Status",
    footer_text: "Loja-Game · Dili, Timor-Leste",

    hero_title: "Top up UC PUBG Global",
    shop_title: "Top up UC PUBG Global",
    hero_title_for: (g) => `Top up ${g}`,
    shop_title_for: (g) => `Top up ${g}`,
    step1: "Pilih paket",
    step2: "Lakukan transfer",
    step3: "Upload bukti transfer",
    step4: "Buat pesanan",
    hero_desc: "Kami akan verifikasi pembayaran dan kirim paket ke game ID Anda (Proses ±15 menit).",
    hero_desc_roblox: "Kami akan verifikasi pembayaran dan mengirim Kode Redeem ke pesanan Anda, silakan cek di menu Cek Status. (Proses ±15 menit).",
    whatsapp_confirm: "Konfirmasi WhatsApp",

    step_indicator_pick: "Pilih Paket",
    step_indicator_pay: "Pembayaran",

    shop_avail: "Tersedia di Timor-Leste",
    promo_1: "Antar gratis di Dili",
    promo_2: "Waktu proses ±15 menit",
    promo_3: "Pembayaran aman — Bank BNCTL / MOSAN",
    trust_delivery: "Proses ±15 Menit",
    trust_secure: "Pembayaran Aman",
    trust_support: "Dukungan WhatsApp",
    search_pkg_placeholder: "Cari paket (cth: 1000)",
    order_history_label: "Pesanan terakhir",
    order_history_empty: "Belum ada pesanan di perangkat ini.",
    select_product_label: "Pilih Produk",
    pkg_count_suffix: "paket",
    out_of_stock_label: "Stok Habis",

    tier_kiik: "Paket Kecil",
    tier_medium: "Paket Menengah",
    tier_boot: "Paket Besar",
    tier_boot_note: "(Proses ±30 menit)",

    checkout_process_time: "Waktu proses: ±15 menit",
    about_title: "Tentang Kami",
    about_story_title: "Cerita Kami",
    about_need_help_title: "Butuh Bantuan?",
    about_need_help_desc: "Jika Anda punya pertanyaan tentang pesanan, pembayaran, atau status pesanan, silakan hubungi kami lewat WhatsApp.",
    about_facebook_label: "Lihat daftar harga kami dari tahun ke tahun →",
    footer_about_label: "Tentang Kami",
    about_paragraphs: [
      "Sejak tahun 2018, kami mulai berjualan UC PUBG dengan cara mengantar langsung ke rumah klien di Dili. Waktu itu, kami belum punya website \u2014 kami hanya menggunakan banner dan daftar harga yang kami perbarui secara berkala mengikuti setiap Royale Pass Season baru, lalu mengirimkannya lewat WhatsApp dan mengantarkan langsung ke tangan klien.",
      "Alasan kuat kami pertama kali berjualan UC adalah karena kami ingin membantu para gamer muda Timor mendapatkan kemudahan cepat untuk upgrade Royale Pass, sama seperti negara-negara lain.",
      "Pada tahun 2020, selama pandemi COVID-19, kami terpaksa menghentikan layanan ini selama satu tahun \u2014 seperti banyak orang lain di Timor-Leste. Tapi setelah keadaan membaik, kami ingin kembali dan melanjutkan layanan ini, karena kami tahu Anda semua masih menunggu kami.",
      "Selama bertahun-tahun ini, kami telah memperbarui daftar harga dan desain kami berkali-kali \u2014 mulai dari Season 4 hingga Season 11 \u2014 selalu berusaha memberikan harga yang adil dan informasi yang jelas kepada pelanggan. Ini bukan bisnis baru atau kepercayaan yang kami dapatkan secara instan; ini adalah hasil dari bertahun-tahun pelayanan konsisten kepada komunitas Dili.",
      "Sekarang, kami membawa layanan ini ke online \u2014 bukan untuk mengubah apa pun, tapi untuk membuat prosesnya lebih mudah bagi Anda dan harganya menjadi lebih terjangkau. Orang yang memverifikasi pesanan Anda adalah orang yang sama yang mengantarkan UC ke rumah-rumah pelanggan selama bertahun-tahun ini.",
      "Di website ini, kami juga telah menambahkan paket untuk Mobile Legends, Free Fire, dan Robux Roblox \u2014 dan kami akan terus menambahkan game-game lain ke depannya, untuk mengikuti kebutuhan komunitas gamer Timor yang terus bertambah."
    ],
    redeem_howto_title: "Cara redeem code Roblox?",
    redeem_step1: "Buka aplikasi Roblox atau roblox.com, lalu login ke akun Anda.",
    redeem_step2: "Di bagian atas, klik ikon kotak/hadiah 📦 atau menu (☰).",
    redeem_step3: "Pilih \"Redeem Roblox Codes\" — atau langsung buka: roblox.com/redeem",
    redeem_step4: "Masukkan kode yang diberikan admin ke kotak \"Code\".",
    redeem_step5: "Klik tombol \"Redeem\".",
    redeem_step6: "Robux akan otomatis masuk ke saldo akun Anda.",

    selected_pkg_label: "Paket dipilih",
    price_per_unit_label: "Harga / unit",
    select_pkg_empty: "Pilih paket",

    user_id_label: "User ID PUBG Global",
    user_id_label_generic: "User ID",
    user_id_placeholder_generic: "Contoh: 123456789",
    zone_id_label: "Zone ID",
    zone_id_placeholder: "Contoh: 2151",
    user_id_placeholder: "Contoh: 1234567891234567",
    user_id_placeholder_suffix: "234567891234567",
    user_id_must_start_5: "User ID harus diawali angka 5.",
    nickname_label: "Nickname",
    nickname_placeholder: "Contoh: ucpubgtl2026",
    quantity_label: "Kuantitas",
    qty_decrease_label: "Kurangi kuantitas",
    qty_increase_label: "Tambah kuantitas",
    fullname_label: "Nama lengkap",
    fullname_placeholder: "Nama Anda",
    whatsapp_label: "Nomor WhatsApp",
    whatsapp_placeholder: "7XXXXXXX",
    add_note_btn: "+ Tambah catatan untuk penjual (opsional)",
    note_label: "Catatan untuk penjual",
    note_placeholder: "Informasi tambahan",

    total_uc_label: "Total",
    subtotal_label: "Subtotal",
    buy_btn: "Beli",
    buy_hint_select_pkg: "Pilih paket dulu untuk melanjutkan.",
    buy_hint_fill_info: "Lengkapi informasi pemain dulu untuk melanjutkan.",

    payment_method_title: "Metode Pembayaran",
    type_bank_transfer: "Transfer Bank",
    type_ewallet: "E-Wallet",

    upload_title: "Upload Bukti Transfer",
    upload_hint: "Transfer pembayaran ke rekening kami, lalu upload bukti transfer di sini.",
    upload_locked_hint: "Pilih metode pembayaran dulu sebelum upload bukti transfer.",
    upload_locked_main: "Pilih metode pembayaran dulu",
    upload_main: "Upload bukti transfer",
    upload_sub: "PNG atau JPG, maks 5MB",
    upload_btn: "Pilih Gambar",
    upload_file_ok: "Gambar telah dipilih",
    upload_change_btn: "Ganti gambar",

    order_info_title: "Informasi Pesanan",
    edit_link: "Edit",

    payment_details_title: "Detail Pembayaran",
    total_order_label: "Total Pesanan",
    total_payment_label: "Total Pembayaran",

    submit_btn: "Kirim Pesanan",
    submitting_label: "Mengirim...",
    submit_error: "Gagal mengirim pesanan. Silakan coba lagi.",
    submit_hint_select_payment: "Pilih metode pembayaran dulu untuk mengirim pesanan.",
    submit_hint_upload_proof: "Upload bukti transfer dulu untuk mengirim pesanan.",

    success_title: "Pesanan Terkirim!",
    success_desc: (currency, wa) => `Kami akan verifikasi pembayaran dan mengirim ${currency} ke game ID Anda. Simpan Order ID ini untuk cek status, dan konfirmasi lewat WhatsApp ${wa}.`,
    success_desc_roblox: (wa) => `Kami akan verifikasi pembayaran dan mengirimkan kode redeem untuk Anda. Simpan Order ID ini, lalu cek di menu Cek Status untuk menerima kode redeem, atau konfirmasi lewat WhatsApp ${wa}.`,
    copy_btn: "Salin",
    copied_btn: "Tersalin",
    ok_btn: "Oke",

    track_eyebrow: "Status Pesanan",
    track_title_line1: "Cek progress",
    track_title_line2: "UC Anda.",
    track_desc: "Masukkan Order ID untuk mencari pesanan Anda. Order ID didapat setelah mengirim pesanan.",
    order_id_label: "Order ID",
    order_id_placeholder_example: (prefix) => `cth. ${prefix}-M2K3X9`,
    optional_label: "(opsional)",
    track_btn: "Cek Status",
    track_loading: "Memuat...",
    track_empty_input: "Silakan masukkan Order ID.",
    track_not_found: "Pesanan tidak ditemukan. Silakan periksa Order ID yang Anda masukkan.",
    track_error: "Gagal memuat pesanan. Silakan coba lagi.",
    track_history_loading: "Memuat riwayat pesanan...",
    track_history_empty: "Belum ada pesanan di perangkat ini. Pesanan yang Anda buat akan muncul di sini.",
    track_contact_whatsapp_btn: "Hubungi WhatsApp",
    unread_badge_label: "Baru",
    notification_bell_label: "Notifikasi",
    notification_bell_label_unread: "Notifikasi — ada pembaruan baru",
    notification_bell_dropdown_title: "Ada pembaruan:",
    update_available_message: "Ada versi baru untuk website ini.",
    update_now_btn: "Update Sekarang",
    close_btn: "Tutup",
    install_ios_title: "Tambahkan Loja-Game ke Layar Utama",
    install_ios_sub: "Tap ikon Share ⬆️ di Safari, lalu pilih \"Add to Home Screen\"",
    install_title: "Install Loja-Game",
    install_sub: "Tambahkan aplikasi ini ke layar utama perangkat Anda",
    install_btn: "Install",
    theme_light_label: "Mode Terang",
    theme_dark_label: "Mode Gelap",
    scroll_to_top_label: "Gulir ke atas",
    order_detail_title: "Detail Pesanan",

    track_explain_menunggu_verifikasi: "Kami sedang memverifikasi pembayaran Anda. Tidak perlu melakukan apa-apa — tunggu saja, dan segarkan halaman ini untuk melihat progress.",
    track_explain_terverifikasi: "Pembayaran Anda telah dikonfirmasi. Kami sedang menyiapkan pengiriman pesanan Anda.",
    track_explain_terkirim: "Pesanan Anda telah selesai dan terkirim. Terima kasih sudah belanja di Loja-Game!",
    track_explain_dibatalkan: "Pesanan ini dibatalkan. Lihat catatan dari penjual di bawah jika ada, atau hubungi kami lewat WhatsApp jika butuh bantuan.",

    pkg_row_label: "Paket",
    price_row_label: "Harga",
    game_id_row_label: "Game ID",
    pubg_name_row_label: "Nickname",
    date_row_label: "Tanggal",
    seller_note_label: "Catatan dari penjual",
    rating_question: "Puas dengan layanan ini?",
    rating_thanks: "Terima kasih atas masukan Anda!",
    rating_comment_prompt: "Bisa kasih tahu apa yang kurang? (opsional)",
    rating_comment_placeholder: "Tulis sedikit di sini...",
    rating_skip_btn: "Lewati",
    rating_send_btn: "Kirim",
    rating_option_bad: "Kurang baik",
    rating_option_neutral: "Netral",
    rating_option_good: "Puas",
    security_pending_limit: "Saat ini ada 3 pesanan Anda yang belum diproses (Menunggu Verifikasi). Mohon tunggu hingga diproses, baru bisa order lagi.",
    security_cancelled_limit: "Hari ini ada 5 pesanan Anda yang Dibatalkan. Anda bisa order lagi besok, atau hubungi admin untuk informasi lebih lanjut.",
    security_blocked: "Perangkat Anda saat ini tidak bisa melakukan pesanan baru. Silakan hubungi admin lewat WhatsApp untuk informasi lebih lanjut.",
    store_closed_banner: (openTime) => `Toko sedang tutup saat ini. Kami akan buka lagi jam ${openTime}. Anda bisa lihat paket dan harga, tapi belum bisa order.`,
    store_closed_error: (openTime) => `Toko sedang tutup. Kami akan buka lagi jam ${openTime}. Silakan coba lagi setelah toko buka.`,
    proof_duplicate_error: (orderId, wa) => `Bukti transfer ini sudah dipakai untuk pesanan ${orderId} yang belum dibatalkan. Kalau ada masalah, silakan hubungi admin lewat WhatsApp ${wa}.`,
    refresh_status_label: "Segarkan status",

    status: {
      menunggu_verifikasi: "Menunggu Verifikasi",
      terverifikasi: "Terverifikasi",
      terkirim: "Terkirim",
      dibatalkan: "Dibatalkan",
    },
  },
}
